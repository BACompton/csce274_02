...
-----------
    ...
    Description:
        ...
        is pressed, the robot's rotation will be counter-clockwise. If both are
    ...
    Execution:
        ...
        If it is let out, the generated log file will be called robot_log.log.
        ...
        After execution is complete, the log file is now safe to open. Within
        the log file, the events and actions that the robot took during the last
        execution will be stored. This file can then be used to plot the robots
        position throughout the programs execution. Do note that on a stopping
        button press or wheel drop, the robot can be moved an indeterminable
        distance from the last point so, the robot's world position need to be
