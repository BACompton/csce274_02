...
-----------
	...
	Description:
		Main.py is the main program for project 2. This program was designed
		...
		robot will respond to the clean/power button being pressed.

		When the clean/power button is pressed and the robot is stopped, it
		will check to make sure none of the Wheel Drops and Cliff are activated.
		If none are activated, the robot will spawn a daemon to control the
		actuators in order to begin to move forward until it reaches an obstacle,
		then will rotate in place for a random amount of time, then will begin
		to move forward again and repeat the process. If the bumper left is
		pressed, the rotation will be clockwise. Likewise, if the bumper right
		is pressed, the robot's rotation will be counterclockwise. If both are
		pressed, the robot will choose a random direction to rotate.
	...
	Execution:
		...
		This will start the main program. Once the text "Listening for press"
		...
		the clean/power button again. To stop the execution of square.py,
		first, ensure the robot is stopped. Next, issue a keyboard
		interrupt(CTRL + C) to the console.
...
----------------
	...
	Description:
		...
		OI such as:
			...
			- Special radii for the drive command (Ex: SRAIGHT, TURN_CW)
	...
	Implementation:
		...
		After creating the object, you have access to the following methods:
			...
			- Read bump (Ex: create2.read_bump(robot_inf.Bump.BUMP_L))
			    NOTE: * Only the values found in "robot_inf.Bump" should
					be used as arguments

			- Read wheel drop (Ex: create2.read_wheel_drop())

			- Read cliff (Ex: create2.read_cliff(robot_inf.Cliff.CLIFF_R))
			    NOTE: * Only the values found in "robot_inf.Cliff" should
					be used as arguments
...
-----------------
	...
	Description:
		...
		iRobot Create 2. This module contains methods that will:
			- Acquire and release reader locks
			- Acquire and release write locks
			- Wake up waiting readers or writers
			- Update sensor values
			- Start and stop currently updating sensor data
			- Check if specified buttons have been pressed/released
	...
	Implementation:
		...
		working directory. This module gives access to the following methods:
			- Acquire read (Ex: _log_sema.acquire()

			- Release read (Ex: _log_sema.release()
...
-----------------
	...
	Description:
		...
		interface will:
			...
			- Send space delimited commands as encoded ASCII commands
			- Read raw data
			- List available serial ports
			- Send and receive the lock
